...
func process_input(_delta):
	...
	# Create a vector for storing our keyboard input
	...
	# Normalize the input movement vector
...
func _input(event):
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		# We need to do this every time we rotate so we cannot rotate upside down with mouse input
...
func bullet_hit(damage, _bullet_global_transform):
